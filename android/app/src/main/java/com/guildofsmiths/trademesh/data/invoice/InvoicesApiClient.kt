...
class InvoicesApiClient(private val client: OkHttpClient) : InvoicesApi {
    ...
    // Bearer token here comes from HttpClientFactory's auth interceptor, which
    // reads AuthService.getAccessToken() -- so a 401 is refreshed via
    // AuthService.refreshToken(), not the SupabaseAuth default (see
    // AuthedRequest's kdoc for why the two must not be conflated).
