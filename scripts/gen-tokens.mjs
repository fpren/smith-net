...
// Avatar palette is theme-invariant (same hues in light and dark), so it is
// emitted once in :root only, not repeated per theme.
function avatarCssVars(pad = '  ') {
  return Object.entries(t.avatar).map(([k, v]) => `${pad}--sn-avatar-${kebab(k)}: ${v};`).join('\n');
}
...
const avatarKeys = Object.keys(t.avatar);
...
${avatarKeys.map((k) => `        'sn-avatar-${kebab(k)}': 'var(--sn-avatar-${kebab(k)})',`).join('\n')}
      },
      boxShadow: {
...
const ktAvatarObj = () =>
  Object.entries(t.avatar).map(([k, v]) => `        val ${k.toUpperCase()} = ${ktColor(v)}`).join('\n');
const avatarPalette = Object.keys(t.avatar).map((k) => `Avatar.${k.toUpperCase()}`).join(', ');
...
${ktAvatarObj()}
    ...
    val AvatarPalette = listOf(${avatarPalette})
