...
- **Design System v2 (Crew Soft / North Cobalt) governs ALL UI** — both the portal and the Android app. Colors ONLY via tokens (`design/tokens.json` → `node scripts/gen-tokens.mjs` → generated `tokens.css`/`tailwind.tokens.cjs`/`Tokens2.kt`; never hand-edit generated files, never hardcode hex). Light-first with a user dark toggle on both platforms. Web: `sn-*` classes, `Smith*`/`ui/` components, state trio on every route, `httpCall` for all API clients. Android: `LocalSmithColors` + colorless `SmithType` (+ explicit color), `theme2/` Smith components; ConsoleTheme's v1 palette is deleted. Crew surfaces are soft (20 radius, pills, shadows); ops surfaces (dispatch/plan/map/admin) are Terminal Grade (0 radius, 1px hairlines, mono-upper, tabular numerals). Glyphs per `design/GLYPHS.md`. Maestro pins visible strings + `solo_e2e_*` tags — grep `android/maestro/` before renaming UI copy. Full rules: `.claude/skills/smith-net-design-system` (+ the frontend/android overlays); spec: `docs/superpowers/specs/2026-07-08-design-system-v2-design.md`.
